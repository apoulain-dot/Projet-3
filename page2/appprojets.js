...
function init() {
  ...
  if (state.columns[0].tasks.length === 0) {
    state.columns[0].tasks.push(
      { id: 'task-1', text: 'Concevoir la maquette du projet', description: 'Créer les wireframes et mockups', priority: 3, assignees: ['member-1'] },
      { id: 'task-2', text: 'Réunion d\'équipe à 10h', description: 'Discuter du planning et des objectifs', priority: 2, assignees: ['member-1', 'member-2'] }
    );

    state.columns[1].tasks.push(
      { id: 'task-3', text: 'Développer les fonctionnalités principales', description: 'API REST, authentification, CRUD', priority: 5, assignees: ['member-2', 'member-4'] },
      { id: 'task-4', text: 'Tests des API', description: 'Tester tous les endpoints', priority: 4, assignees: ['member-3'] }
    );
...
function setupModal() {
  ...
  // Mettre à jour l'indicateur de priorité en temps réel
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (taskText && state.currentColumnId) {
      if (editingTaskId) {
        updateTask(editingTaskId, taskText, taskDesc, priority, assignees);
      } else {
        addTask(state.currentColumnId, taskText, taskDesc, priority, assignees);
      }
      closeModal();
    } else {
...
function applyMaxPriorityToSlider() {
    const slider = document.getElementById('taskPriority');
    const priorityValue = document.getElementById('priorityValue');
    if (!slider) return;

    slider.max = String(MAX_PRIORITY);
    slider.min = String(MIN_PRIORITY);

    
    const val = parseInt(slider.value || DEFAULT_PRIORITY, 10);
    slider.value = Math.min(Math.max(val, MIN_PRIORITY), MAX_PRIORITY);
    if (priorityValue) priorityValue.textContent = slider.value;
}

// S'assurer que toutes les tâches existantes respectent la nouvelle plage
function clampExistingTaskPriorities() {
    state.columns.forEach(col => {
        col.tasks.forEach(t => {
            if (typeof t.priority !== 'number' || isNaN(t.priority)) {
                t.priority = DEFAULT_PRIORITY;
            }
            t.priority = Math.min(Math.max(t.priority, MIN_PRIORITY), MAX_PRIORITY);
        });
    });
}

// Appliquer immédiatement et à chaque ouverture du modal
applyMaxPriorityToSlider();
clampExistingTaskPriorities();

// Si le modal est ouvert plus tard, forcer la mise à jour du slider à l'ouverture
const originalOpenModal = openModal;
openModal = function (columnId, taskId = null) {
    applyMaxPriorityToSlider();
    // appeler l'original
    return originalOpenModal(columnId, taskId);
};
// Fermer le modal
function closeModal() {
  const modal = document.getElementById('taskModal');
...
function createTaskElement(task, columnId) {
  ...
  // Clic pour éditer la tâche
  div.addEventListener('click', (e) => {
    if (!e.target.closest('.btn-delete-task') && !e.target.closest('.task-avatar')) {
...
// Ajouter une tâche
function addTask(columnId, text, description, priority, assignees) {
...
// Modifier une tâche
function updateTask(taskId, text, description, priority, assignees) {
  let task = null;
  state.columns.forEach(col => {
    const found = col.tasks.find(t => t.id === taskId);
    if (found) task = found;
